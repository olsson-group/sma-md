...
class TorsionNoiseTransform(BaseTransform):
    def __init__(self, sigma_min=0.01 * np.pi, sigma_max=np.pi, boltzmann_weight=False, n_conformations=1):
    ...
    def __call__(self, data):
        ...
        #to do: select n_conformations and create a list of data objects with its corresponding conformation. Collate them with https://pytorch-geometric.readthedocs.io/en/latest/generated/torch_geometric.data.Batch.html
        if self.boltzmann_weight:
            data.pos = random.choices(data.pos, data.weights, k=self.n_conformations)
        else:
            data.pos = random.choices(data.pos, k=self.n_conformations)

        data_list = []
        for conformation in data.pos:
            data_copy = copy.deepcopy(data)
            data_copy.pos = conformation
            
            try:
                edge_mask, mask_rotate = data_copy.edge_mask, data_copy.mask_rotate
            except:
                edge_mask, mask_rotate = data_copy.mask_edges, data_copy.mask_rotate
                data_copy.edge_mask = torch.tensor(data_copy.mask_edges)

            sigma = np.exp(np.random.uniform(low=np.log(self.sigma_min), high=np.log(self.sigma_max)))
            data_copy.node_sigma = sigma * torch.ones(data_copy.num_nodes)

            torsion_updates = np.random.normal(loc=0.0, scale=sigma, size=edge_mask.sum())
            data_copy.pos = modify_conformer(data_copy.pos, data_copy.edge_index.T[edge_mask], mask_rotate, torsion_updates)
            data_copy.edge_rotate = torch.tensor(torsion_updates)
