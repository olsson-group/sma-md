...
directories = ["metrics", "metrics/rmsd/", "metrics/rmsd/precision/", f"metrics/rmsd/precision/{source}_{reference}/"]
if source == "md_tuned":
    directories.append(f"metrics/rmsd/precision/{source}_{reference}/{model_name}/")
...
for dataset_idx in tqdm(indices):
    ...
    reference_conformers, reference_atoms = get_conformers(referece_z_matrices, rb_atoms, mol_dic, threshold=0.05)
    source_conformers, source_atoms = get_conformers(source_z_matrices, rb_atoms, mol_dic, threshold=0.05)
    ...
    for i_conf, source_conformer in enumerate(source_conformers):
    ...
    if source == "md_tuned":
        np.save("metrics/rmsd/precision/{:s}_{:s}/{:s}/min_rmsd_{:0>5d}.npy".format(source, reference, model_name, dataset_idx), min_rmsd)
        np.save("metrics/rmsd/precision/{:s}_{:s}/{:s}/min_indices_{:0>5d}.npy".format(source, reference, model_name, dataset_idx), min_indices)
    else:
        np.save("metrics/rmsd/precision/{:s}_{:s}/min_rmsd_{:0>5d}.npy".format(source, reference, dataset_idx), min_rmsd)
        np.save("metrics/rmsd/precision/{:s}_{:s}/min_indices_{:0>5d}.npy".format(source, reference, dataset_idx), min_indices)
