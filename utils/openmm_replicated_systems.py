"""
Addapted from https://github.com/noegroup/reform/blob/master/reform/omm_replicated.py
...
class OMMTReplicas_replicated(OMMTReplicas):
    ...
    def get_energy_contributions(self, index: int):
        """Return the potential energy of replica #index."""
        self._check_index(index)
        n_contributions = len(self._system.getForces())
        energy_contributions = unit.Quantity(value = np.zeros(n_contributions), unit = unit.kilojoule_per_mole)
        self._single_context.setPositions(self.get_positions(index))
        for i_contribution, f in enumerate(self._system.getForces()):
            state: omm.State = self._single_context.getState(getEnergy=True, groups={i_contribution})
            energy_contributions[i_contribution] = state.getPotentialEnergy()
